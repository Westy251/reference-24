...
/*Should a function similar to "load initial firmware" be added here?*/

/* OLD BRUGH CODE*
* UART READ BYTES
*
* Reads a given umber of bytes from the host
*
* \param bytes is the number of bytes to be read
* \param dest is where to write them
*
* \return Returns 0 if reading is successful, 1 if not
*/
...
* FRAME DECRYPT
*
* \param arr is the array that unecrypted data will be written to
...
int frame_decrypt(uint8_t *arr, int expected_type){
    ...
    unsigned char gen_hash[32]; //ghash?
    ...
    // Read and check TYPE
    if (uart_read(UART0, BLOCKING, &read) != (int) expected_type){
        error = 1;
        return error;
    }

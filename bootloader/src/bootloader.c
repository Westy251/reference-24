...
int main(){
    ...
    char ciphertext[] = {0xe6,0x4d,0x52,0x79,0x38,0xe4,0x34,0xba,0x6b,0x27,0xd3,0xdf,0xdd,0x3a,0xa9,0x8d,0x6e,0xfd,0xd4,0x17,0x16,0xb0,0xc4,0x89,0x34,0xe7,0x5a,0xee,0x25,0xb6,0xea,0xdb,0x1a,0x83,0xdd,0xcf,0x19,0xc5,0xd2,0xf9,0xf3,0xc7,0x71,0xa2,0x5b,0x25,0xd4,0x11};

    uint8_t size = sizeof(ciphertext);
 
    wc_AesSetIV(&dec, iv); // Set IV

    wc_AesSetKey(&dec, KEY, 16, iv, AES_DECRYPTION); //Set KEY

    char plaintext[size]; // Define plaintext
    wc_AesCbcDecrypt(&dec, plaintext, ciphertext, size); // Decrypt
    
    for (int i = 0; i < size; i++){uart_write_hex(UART0, plaintext[i]);} // Print plaintext
